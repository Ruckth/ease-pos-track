...
export interface KanbanColumnProps extends useRender.ComponentProps<"div"> {
  ...
  disabled?: boolean
  /**
   * Set to false for a fixed board whose columns must never be reordered. The
   * column stays a drop target for cards; only its own draggable is switched off,
   * so no sensor can pick the column up even if a handle is rendered by mistake.
   */
  draggable?: boolean
}
...
function KanbanColumn({
  ...
  const {
    ...
    id: value,
    // `disabled` blocks dragging and dropping; `draggable={false}` keeps the
    // column droppable so cards can still be dropped onto it.
    disabled: disabled || isOverlay ? true : draggable ? false : { draggable: true },
...
function KanbanItem({
  ...
  const defaultProps = isOverlay
    ...
    : {
        ...
        ref: setNodeRef,
        style,
        // The dnd-kit `attributes` (role, tabIndex, aria-roledescription,
        // aria-describedby) belong on the activator, not here: a card is only
        // draggable through its KanbanItemHandle, and announcing the whole card as
        // a focusable "draggable button" both nests interactive controls and
        // promises a keyboard drag the card body cannot start. KanbanItemHandle
        // spreads them instead, matching KanbanColumnHandle.
        className: cn(
          isSortableDragging && "opacity-50 z-50",
          disabled && "opacity-50",
          className
...
function KanbanItemHandle({
  ...
  const defaultProps = {
    ...
    "data-dragging": isDragging,
    "data-disabled": disabled,
    // The handle is the drag activator, so it carries dnd-kit's aria attributes
    // and is registered as the activator node (used for focus restoration and the
    // keyboard sensor).
    ...attributes,
    ...listeners,
    className: cn(
