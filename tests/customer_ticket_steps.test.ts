...
test("step two needs at least one photo or video", () => {
  assert.deepEqual(stepIssues(2, titled), ["REQUIRED_MEDIA"]);
...
test("later steps stay closed until the earlier ones are complete", () => {
  ...
  assert.equal(canEnterStep(3, titled), false);
...
test("the active step is clamped to what the draft has unlocked", () => {
  ...
  assert.equal(highestReachableStep(titled), 2);
  assert.equal(highestReachableStep(withMedia), 3);

  assert.equal(clampStep(3, empty), 1);
  assert.equal(clampStep(3, titled), 2);
  assert.equal(clampStep(3, withMedia), 3);
  assert.equal(clampStep(0, withMedia), 1);
  assert.equal(clampStep(9, ready), 3);
});

test("clearing an earlier step pulls the flow back", () => {
  // On review, then the media is removed again.
  assert.equal(clampStep(3, { ...withMedia, mediaCount: 0 }), 2);
...
test("submission requires every gate to pass", () => {
  ...
  assert.equal(canSubmitDraft(titled), false);
